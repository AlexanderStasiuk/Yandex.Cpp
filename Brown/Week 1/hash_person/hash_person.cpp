...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        ...
        return  str_hasher(address.city) * (coef*coef) +
                str_hasher(address.street) * (coef) + 
...
struct PersonHasher {
    size_t operator() (const Person& person) const {
        ...
        const size_t coef = 2'946'901;
        return  str_hasher(person.name) * (coef*coef*coef) +
                int_hasher(person.height) * (coef*coef) + 
                double_hasher(person.weight) * (coef) +
