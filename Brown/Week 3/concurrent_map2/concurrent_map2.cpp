...
public:
    ...
    WriteAccess operator[](const K& key) {
        const size_t bucket_index = GetIndex(key);

	    return WriteAccess { lock_guard(buckets_[bucket_index].m), buckets_[bucket_index].part_map_[key] };
    }

    ReadAccess At(const K& key) const {
        if (!Has(key)) {
            throw out_of_range("ked doesn't exist in map!");
        }

        const size_t bucket_index = GetIndex(key);

        return ReadAccess { lock_guard(buckets_[bucket_index].m), buckets_[bucket_index].part_map_.at(key) };
    }

    bool Has(const K& key) const {
        const size_t bucket_index = GetIndex(key);
        lock_guard guard(buckets_[bucket_index].m);
    ...
    std::unordered_map<K, V, Hash> BuildOrdinaryMap() const {
        std::unordered_map<K, V, Hash> full_map;
        for (size_t i = 0; i < buckets_.size(); ++i) {
            lock_guard guard(buckets_[i].m);
...
private:
    struct Bucket {
        std::unordered_map<K,V,Hash> part_map_;
        mutable mutex m;
    };
