...
public:
    ...
    int FindNearestFinish(int start, int finish) const {
        ...
        if (!reachable_stations.empty()) {
            // cerr << endl;
            // for (auto i: reachable_stations) {
            //     cerr << i << ", ";
            // }
            const set<int>::iterator pre = reachable_stations.lower_bound(finish);
            if (pre != end(reachable_stations)) {
                result = min(result, abs(finish-*pre));
            }
            if (pre != begin(reachable_stations)) {
                result = min(result, abs(finish-*prev(pre)));
            }

            // cerr << "\nStart: " << start << " Finish: " << finish << " - " << val << endl;
