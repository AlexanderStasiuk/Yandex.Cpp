...
public:
  ...
  // конструктор от диапазона элементов типа TAirport
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        auto iter = begin;
        while (iter != end) {
            data_[static_cast<uint32_t>(*iter)]++;
            iter++;
        }
    }
  ...
  // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        // cout << static_cast<uint32_t>(airport) << " : " << data_[static_cast<uint32_t>(airport)] << endl;
        return data_[static_cast<uint32_t>(airport)];
    }
  ...
  // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) {
        if (data_[static_cast<uint32_t>(airport)] > 0) {
            data_[static_cast<uint32_t>(airport)]--;
        }
    }
...
private:
    const size_t size_ = static_cast<uint32_t> (TAirport::Last_);   
